...
object DrawingBarTestTags {
    ...
    const val COLOR_WIDTH_SECTION_ROOT = "drawing_bar:color_width_section_root" // Root Column of the ColorAndWidth composable

    const val COLOR_SELECTOR_LAYOUT = "drawing_bar:color_selector_layout" // FlowLayout2 for colors
    const val COLOR_SELECTOR_ITEM_PREFIX = "drawing_bar:color_item"   // Base for individual color Box tags (append index in tests)

    const val WIDTH_SELECTOR_LAYOUT = "drawing_bar:width_selector_layout"   // Row for widths
    const val WIDTH_SELECTOR_ITEM_PREFIX = "drawing_bar:width_item"     // Base for individual width Box tags (append index in tests)
}
