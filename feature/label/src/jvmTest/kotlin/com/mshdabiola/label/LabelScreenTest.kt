...
class LabelScreenTest {
    ...
    fun newLabelInput_iconChangesWithText_andClearButtonWorks_whenFocused() {
        ...
        setupLabelScreen(
            initialUiState =
            LabelUiState(newLabel = newLabelState, labels = emptyList(), isEditMode = true),
        )
        composeTestRule.waitForIdle() // Allow LaunchedEffect and state updates

        // Initial state: newLabel is focused (isCurrentFocus=true
        // in EditLabelTextField due to currentFocus=-1), text is empty.
        // -> Add icon should be displayed.
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .NEW_LABEL_ADD_ICON_INDICATOR,
            useUnmergedTree = true,
        ).assertIsDisplayed()
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .NEW_LABEL_CLEAR_BUTTON,
        ).assertDoesNotExist()
        ...
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .NEW_LABEL_DONE_BUTTON,
        ).assertDoesNotExist() // Done button hides
    }
    ...
    fun newLabelInput_typeAndClickDone_invokesOnAdd() {
        ...
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .NEW_LABEL_INPUT,
        ).performClick() // Click to focus. onFocused -> currentFocus = -1.
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .NEW_LABEL_INPUT,
    ...
    fun existingLabel_clickEdit_focusesAndChangesIcons() {
        ...
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .itemLabelIconIndicator(labelId),
            useUnmergedTree = true,
        )
            .assertIsDisplayed()
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .itemEditButton(labelId),
            useUnmergedTree = true,
        ).assertIsDisplayed()
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .itemDeleteButton(labelId),
            useUnmergedTree = true,
        ).assertDoesNotExist()
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .itemDoneButton(labelId),
            useUnmergedTree = true,
        ).assertDoesNotExist()
    ...
    fun existingLabel_editTextAndClickDone_invokesOnAdd() {
        ...
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .itemEditButton(labelId),
            useUnmergedTree = true,
        ).performClick() // Sets currentFocus to itemIndex
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .itemLabelInput(labelId),
            useUnmergedTree = true,
        )
    ...
    fun existingLabel_clickDelete_invokesOnDelete() {
        ...
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .itemEditButton(labelIdToDelete),
        ).performClick() // Sets currentFocus to itemIndex
...
//        composeTestRule.onNodeWithTag(LabelScreenTestTags.itemDeleteButton(labelIdToDelete)).assertDoesNotExist()
        ...
        composeTestRule.onNodeWithTag(
            LabelScreenTestTags
                .itemDeleteButton(labelIdToDelete),
        ).performClick()
...
//        composeTestRule.onNodeWithTag(LabelScreenTestTags.NEW_LABEL_ADD_ICON_INDICATOR).assertIsDisplayed()
    ...
    fun screen_whenNoExistingLabels_onlyNewLabelInputIsShownInListContent() {
        ...
        // This is a bit indirect; a more robust way might be to check the count
        // if LazyColumn supports it directly in test.
