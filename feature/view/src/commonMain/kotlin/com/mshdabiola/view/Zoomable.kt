    ...
    val onTransformStopped: (velocity: Velocity) -> Unit = { velocity ->
        if (state.isReadyForInteraction) {
            coroutineScope.launch {
                ...
                if (boundaryState is RealZoomableState.OverzoomBoundaryState.WithinBounds) {
                    state.fling(velocity = velocity, density = requireDensity())
                } else {
                    val hapticType = when (boundaryState) {
                        RealZoomableState.OverzoomBoundaryState.OverZoomed -> state.zoomSpec.maximum.overzoomEffect.hapticFeedbackType()
                        RealZoomableState.OverzoomBoundaryState.UnderZoomed -> state.zoomSpec.minimum.overzoomEffect.hapticFeedbackType()
                        RealZoomableState.OverzoomBoundaryState.WithinBounds -> error("unreachable code")
                    }
